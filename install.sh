...
echo "  Flags: -c canal  -p prioridad (low/normal/high)  -t titulo"
echo ""
echo "  jaimito send \"Hola desde el VPS\""
echo "  jaimito send -c deploys -p high \"Deploy exitoso\""
echo "  jaimito send -t \"Deploy\" \"v1.2 desplegado en produccion\""
echo "  jaimito send --tags backup,cron \"Backup completado\""
echo ""
echo -e "${CYAN}Desde stdin (pipes):${NC}"
echo "  df -h / | jaimito send --stdin -t \"Disco\" -c monitoring"
echo "  tail -20 /var/log/syslog | jaimito send --stdin -c system"
echo "  docker ps --format \"table {{.Names}}\t{{.Status}}\" | jaimito send --stdin -t \"Containers\""
echo "  { uptime; free -h | head -2; } | jaimito send --stdin -t \"Estado del VPS\""
echo ""
echo -e "${CYAN}Monitorear cron job (notifica solo si falla):${NC}"
echo "  jaimito wrap -- /path/to/script.sh"
echo "  jaimito wrap -c cron -p high -- certbot renew"
echo ""
...
echo "  jaimito metric -n disk_root --value 85 # Ingestar metrica manual"
echo "  http://127.0.0.1:8080/dashboard       # Dashboard web"
echo ""
echo -e "${CYAN}Configurar metricas:${NC}"
echo "  Las metricas se definen en la seccion 'metrics' de config.yaml."
echo "  Si no esta presente, el collector no arranca (metricas desactivadas)."
echo ""
echo "  1. Editar config:  sudo nano ${CONFIG_FILE}"
echo "  2. Agregar la seccion 'metrics' (ver ejemplo abajo o en configs/config.example.yaml)"
echo "  3. Reiniciar:      sudo systemctl restart ${SERVICE_NAME}"
echo "  4. Verificar:      jaimito status"
echo ""
echo "  Ejemplo minimo para agregar al final de ${CONFIG_FILE}:"
echo ""
echo "    metrics:"
echo "      retention: \"7d\""
echo "      alert_cooldown: \"30m\""
echo "      collect_interval: \"60s\""
echo "      definitions:"
echo "        - name: disk_root"
echo "          command: \"df / | awk 'NR==2 {print \$5}' | tr -d '%'\""
...
echo "            critical: 90"
echo ""
echo "  Mas ejemplos (sistema, docker, postgres, redis):"
echo "    cat ${REPO_DIR}/configs/config.example.yaml"
echo ""
