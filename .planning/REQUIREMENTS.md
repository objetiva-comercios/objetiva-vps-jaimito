...
- [x] **CLI-01**: `jaimito send "message"` sends a notification with optional -c channel, -p priority, --stdin flags
- [x] **CLI-02**: `jaimito wrap -c channel -- /path/to/cmd` runs a command and notifies on failure with exit code and output
...
- [x] **CONF-03**: Service ships with a systemd unit file for Linux deployment
